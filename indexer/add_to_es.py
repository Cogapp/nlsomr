...
def insert_into_elasticsearch(id, json):
    doc_id = id
#    doc = {
#      'test': 'test',
#      'doc_id': 'doc_id',
        ...
        if not os.path.isfile(BASE_XML_PATH +"/"+ book): #we have a directory
            for page in listdir(BASE_XML_PATH +"/"+ book):
                if not os.path.isfile(BASE_XML_PATH+"/"+book+"/"+page):
                    print ("PAGE:"+page)
                    for f in listdir(BASE_XML_PATH+"/"+book+"/"+page):
                        if f[-8:] == '_extract':
                            filename = '{}/{}/{}.xml'.format(BASE_XML_PATH+"/"+book+"/"+page, f, f[:-8])
                            json = document_from_xml(book, page, filename)
                            if json != '':
                                insert_into_elasticsearch(f[:-8], json)
