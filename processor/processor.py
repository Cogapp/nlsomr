                ...
                if os.path.isfile(inputFolder+book+"/"+inputfile) and (inputfile != ".DS_Store"):
                    ...
                    for outputItem in listdir(outputFolder + book +"/"+ os.path.splitext(inputfile)[0]):
                        ...
                        if os.path.splitext(outputItem)[1] == ".mxl": #there may be multiple mxl files generated so we skip
                            os.system("unzip -o -d %s%s/%s/%s_extract %s%s/%s/%s" % (outputFolder, book, os.path.splitext(inputfile)[0],os.path.splitext(outputItem)[0], outputFolder, book, os.path.splitext(inputfile)[0],outputItem)) #we have to extract the mxl bundle for musescore to be able to process it and the indexer to index it
                            for extractFile in listdir(outputFolder + book +"/"+ os.path.splitext(inputfile)[0]+"/"+os.path.splitext(outputItem)[0]+"_extract"):
                                if (os.path.splitext(extractFile)[1] == ".xml") and not (os.path.isfile(wavFolder + os.path.splitext(extractFile)[0] + ".wav")):
